...
```{r}
plot(starbucks$carb,starbucks$calories)
```


3. Add the regression line for calories regressed on carbohydrates
```{r}
lm1 <- lm(calories ~ carb, data=starbucks)
plot(starbucks$carb,starbucks$calories)
abline(lm1)
```
...
```{r}
which(abs(student_resids) > 2)
plot(starbucks$carb,starbucks$calories,col=(abs(student_resids)>2)+1)
abline((lm1))
...
#which values have high influence according to cooks distance?
which(cooksd>(4/nrow(starbucks)))

#which values have high influence according to diffits?
p=2 #2 params in model: b0 and b1
which(abs(diffits)>(2*sqrt(p/nrow(starbucks))))

#plot them with colors representing flagged points
plot(starbucks$carb, starbucks$calories,
     col=(cooksd>(4/nrow(starbucks)))+
       (abs(diffits)>(2*sqrt(p/nrow(starbucks))))+1)
abline(lm1)
legend("bottomright", c("dffits", "dffits+cooks"), col=c(2,3), pch=1)
```
